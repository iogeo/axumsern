...
async fn root(ws: WebSocketUpgrade) -> impl IntoResponse{
    ws.on_upgrade(move |mut sock| async move{
        ...
        q += "file end.mp4\n";
        fs::write(format!("qw{}/concat", rq), &q.to_string().as_bytes()).unwrap();
        let qww = Command::new("sh")
            .arg("-c")
            .arg(format!("ffmpeg -f concat -safe 0 -i ./qw{}/concat -preset ultrafast ./qw{}/final.mp4", rq, rq))
            .output()
            .unwrap();
        let mut r = File::open(format!("./qw{}/final.mp4", rq)).unwrap();
        let mut p = vec![];
        r.read_to_end(&mut p);
